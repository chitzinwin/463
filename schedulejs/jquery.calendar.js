				...
				if( !drag.started && ( drag.incY > 5 || drag.incY < -5 || drag.incX > 5 || drag.incY < -5 ) ){
					...
					switch( data.type ){
						...
						case const_week :
							...
							var resourceMovement	= Math.round( drag.incX / data.cache.resourceWidth );
							...
							// console.log(data.settings.dragincrement+" "+incrementMovement);
							
							console.log(dayMovement);

							//Increment by the resource
							// var testTimeB	= drag.bTime.addDays(0).incrementBy( data.settings.dragincrement, 0-incrementMovement );
							// var testTimeE	= drag.eTime.addDays(0).incrementBy(data.settings.dragincrement, 0-incrementMovement );

												// We've incremented our date, so remove this from the increment counter.
							drag.incY -= (incrementMovement*data.cache.dragHeight);
							drag.incX -= (dayMovement*data.cache.dayWidth);

							// drag.incX -= (resourceMovement*data.cache.resourceWidth);
